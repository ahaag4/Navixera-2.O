(function () {
  ...
  const loadUserData = (uid) => {
    db.ref(`users/${uid}`).on('value', snap => {
      userData = snap.val() || { points: 0, history: [], name: '', emergencyContacts: [] };
  ...
  const saveProfile = () => {
    ...
    db.ref(`users/${currentUser.uid}`).update({ name, emergencyContacts: contacts })
      .then(() => {
  ...
  const showHistory = () => {
    dom.historyList.innerHTML = (userData.history || []).map(h => `
      <li class="list-group-item">${h.type}: ${h.details} on ${new Date(h.timestamp).toLocaleString()}</li>
    `).join('') || '<li class="list-group-item text-muted">No history yet.</li>';
  ...
  const dailyCheckIn = () => {
    ...
    if (Date.now() - lastCheckIn > 86400000) { // 24 hours
      ...
      db.ref(`users/${currentUser.uid}`).update({ points: newPoints, lastCheckIn: Date.now() })
  ...
  const buyTicket = async () => {
    ...
    const from = dom.ticketFrom.value;
    const to = dom.ticketTo.value;
    const bus = dom.ticketBusSelect.value;
  ...
  const sendSafetyAlert = () => {
    ...
    // Mock: Send to police and contacts
